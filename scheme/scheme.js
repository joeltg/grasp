...
BiwaScheme.undef.toString = () => "#<undef>";
...
BiwaScheme.Class = {
  create(methods) {
    const klass = () => this.initialize.apply(this, arguments);
...
//     }
//   ...
BiwaScheme.Class.memoize = (klass, name_or_names) => {
  const proto = klass.prototype;
  const names = _.isArray(name_or_names) ? name_or_names : [name_or_names];
  ...
  if (_.isString(obj))
    return '"' +
           obj.replace(/\\|\"/g, $0 => '\\'+$0)
              ...
              .replace(/\r/g, "\\r") +
           '"';
  if (_.isArray(obj) && obj.closure_p)
    return "#<Closure>";
  if (_.isArray(obj))
    return "#(" + _.map(obj, e => BiwaScheme.to_write(e)).join(" ") + ")";
  if (typeof(obj.to_write) == 'function')
    return obj.to_write();
...
with(BiwaScheme) {
  ...
  define_libfunc("map-with-index", 2, null, function(ar){
    ...
    return Call.multi_foreach(lists, {
      ...
      finish: function(){ return array_to
    ';; Functions are green.\n' +_list(results); }
    });
  });
