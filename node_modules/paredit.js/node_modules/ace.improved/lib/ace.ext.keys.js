...
function flattenedCommandKeyBindings(allBindings) {
  var bound = allBindings.map(keysAndCommandsOfCommandKeyBinding).flatten();
  return bound.reduce(function(map, cmdBnd) {
    var existing = map[cmdBnd.cmdName];
    if (!existing) map[cmdBnd.cmdName] = [];
    map[cmdBnd.cmdName].push(cmdBnd);
    return map;
  }, {})
}
...
function keysAndCommandsOfCommandKeyBinding(b) {
  return Object.keys(b).map(function(k) {
      var cmd = b[k];
      var keys = k.replace(/cmd-/i, "command-").toLowerCase();
      if (keys === "ctrl-p")debugger;
...
function allCommandKeyBindings(ed) {
  return [ed.commands.commandKeyBinding].concat(
    ed.keyBinding.$handlers.map(function(h) {
      if (!h.isEmacs) return h.commandKeyBinding;
      return Object.keys(h.commandKeyBinding).reduce(function(bnd, k) {
        var sanitized = k.replace(/cmd-/i, "command-").replace(/c-/i, "ctrl-").replace(/m-/i, "alt-").replace(/s-/i, "shift-").toLowerCase();
...
KeyHandlerForCustomizations.prototype.handleKeyboard = function(data, hashId, keyString, keyCode) {
  var key = keys.KEY_MODS[hashId] + keyString;
  var command = this.commandKeyBinding[key];
  if (data.$keyChain) {
      command = this.commandKeyBinding[data.$keyChain + " " + key] || command;
  }
...
function ensureKeyBindingCustomization() {
  ...
  KeyBinding.prototype.$callKeyboardHandlers = function(hashId, keyString, keyCode, e) {
    ...
    if (custHandlers) {
      var modeId = this.$editor.session.getMode().$id;
      if (modeId)
        custHandlers = custHandlers.filter(function(ea) {
          return !ea.bindingsSpec.modes || ea.bindingsSpec.modes.indexOf(modeId) > -1; });
      this.$handlers = this.$handlers.concat(custHandlers);
    }
    try {
      return this.$callKeyboardHandlersUnmodified(hashId, keyString, keyCode, e);
    } finally {
      if (custHandlers) {
...
function addKeyCustomizationLayer(layerName, bindingsSpec) {
  ...
  var handler = bindingsSpec.keyHandler || new KeyHandlerForCustomizations(bindingsSpec.commandKeyBinding);
  fixInputBindings(handler);
